...
const History = () => {
  ...
  const handleDeleteCard = async (id: string) => {
    try {
      setDeletingId(id)
      await axios.delete(`${process.env.NEXT_PUBLIC_BACKEND_URL}/content/${id}`, {
        headers: { Authorization: `Bearer ${await getToken()}` },
      })
      setHistory((prev) => prev.filter((item) => item._id !== id))
    } catch (error) {
    ...
    <div className="">
      ...
      <div className="mx-auto max-w-7xl">
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="relative w-full">
              ...
              <Input
                ...
                onChange={(e) => setSearch(e.target.value)}
                placeholder="Search by title or content..."
                className="w-full rounded-xl border border-slate-200 bg-slate-50 py-2 pl-9 pr-4 text-sm text-gray-700 placeholder:text-gray-400 focus:border-teal-500 focus:ring-1 focus:ring-teal-500 focus:outline-none dark:border-slate-600 dark:bg-slate-700/50 dark:text-white dark:placeholder:text-slate-400"
              />
            </div>
            ...
            <div className="flex items-center gap-2 overflow-x-auto pb-1 sm:flex-wrap sm:overflow-visible sm:pb-0">
              ...
              {PLATFORM_BUTTONS.map((button) => (
                ...
                >
                  ...
                  <span className="hidden xs:inline sm:inline">{button.title}</span>
                </Button>
              ))}
            </div>
          </div>
        ...
        <div className="mt-4 space-y-3 sm:mt-5 sm:space-y-4">
          ...
          {/* Loading */}
          {loading && (
            <div className="rounded-2xl border border-slate-100 bg-white p-10 text-center shadow-sm dark:border-slate-700 dark:bg-slate-800 sm:p-12">
              <div className="mx-auto mb-4 h-8 w-8 animate-spin rounded-full border-[3px] border-teal-500 border-t-transparent" />
              <p className="text-sm font-medium text-slate-400">Loading your content history...</p>
            </div>
          )}
          ...
          {/* Empty */}
          {!loading && filtered.length === 0 && (
            <div className="rounded-2xl border border-dashed border-slate-200 bg-white p-10 text-center shadow-sm dark:border-slate-600 dark:bg-slate-800 sm:p-12">
              <div className="mx-auto mb-4 flex h-14 w-14 items-center justify-center rounded-2xl bg-slate-100 dark:bg-slate-700">
                <Sparkles className="h-6 w-6 text-slate-400" />
              </div>
              <p className="text-base font-semibold text-slate-700 dark:text-white sm:text-lg">
                {search || activeFilter ? 'No results found 🔍' : 'No history yet 🚀'}
              </p>
          ...
          {!loading &&
            filtered.map((item) => (
              <Link href={`/results/${item._id}`} key={item._id}>
                <div className="group my-3 overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-sm transition-all duration-200 hover:-translate-y-px hover:border-teal-200 hover:shadow-md dark:border-slate-700 dark:bg-slate-800 dark:hover:border-teal-700">
                  <div className="p-4 sm:p-5">

                    {/* Main card row */}
                    <div className="flex items-start gap-3 sm:gap-4">

                      {/* Thumbnail / placeholder — fixed size */}
                      <div className="relative h-12 w-12 shrink-0 overflow-hidden rounded-xl sm:h-14 sm:w-14">
                        {item.imageUrl ? (
                          ...
                          <div className="flex h-full w-full items-center justify-center rounded-xl border border-slate-100 bg-gradient-to-br from-teal-50 to-slate-100 dark:border-slate-600 dark:from-teal-900/30 dark:to-slate-700">
                            <ImageIcon className="h-5 w-5 text-teal-400/60" />
                          </div>
                        )}
                      </div>
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="flex items-start justify-between gap-2">
                          <h3 className="line-clamp-2 text-sm font-semibold leading-snug text-slate-800 transition-colors group-hover:text-teal-600 dark:text-white sm:text-[15px]">
                          ...
                          <button
                            ...
                            className="shrink-0 rounded-lg p-1.5 text-slate-400 transition hover:bg-red-50 hover:text-red-500 dark:hover:bg-red-900/20 dark:hover:text-red-400 disabled:opacity-40"
                            title="Delete"
                          >
                            {deletingId === item._id ? (
                        ...
                        {/* Meta badges — wraps naturally */}
                        <div className="mt-2 flex flex-wrap gap-1.5 sm:gap-2">
                          <span className="inline-flex items-center gap-1 rounded-lg bg-slate-100 px-2 py-0.5 text-xs font-medium text-slate-600 dark:bg-slate-700 dark:text-slate-300 sm:px-2.5 sm:py-1 capitalize">
                            🎯 {item.tone}
                          </span>
                          <span className="inline-flex items-center gap-1 rounded-lg bg-slate-100 px-2 py-0.5 text-xs font-medium text-slate-600 dark:bg-slate-700 dark:text-slate-300 sm:px-2.5 sm:py-1 capitalize">
                            👥 {item.audience}
                          </span>
                          <span className="inline-flex items-center gap-1 rounded-lg bg-teal-50 px-2 py-0.5 text-xs font-medium text-teal-600 dark:bg-teal-900/40 dark:text-teal-400 sm:px-2.5 sm:py-1 capitalize">
                            <Sparkles className="h-3 w-3 text-teal-400" />
                            {item.creditsUsed} Credits
                          </span>
                          <span className="inline-flex items-center gap-1 rounded-lg bg-slate-100 px-2 py-0.5 text-xs text-slate-500 dark:bg-slate-700 dark:text-slate-400 sm:px-2.5 sm:py-1 capitalize">
                            📅{' '}
                            {new Date(item.createdAt).toLocaleDateString('en-IN', {
                              day: 'numeric',
                              month: 'short',
                        ...
                        <div className="mt-2 flex flex-wrap gap-1.5">
                          {item.platforms?.map((p) => (
                            <span
                              key={p}
                              className={`rounded-full border border-transparent px-2.5 py-0.5 text-xs font-semibold capitalize ${getPlatformColor(p)}`}
                            >
                              {p}
                            </span>
                          ))}
                  ...
                  {/* Bottom accent line on hover */}
                  <div className="h-0.5 w-0 bg-gradient-to-r from-teal-400 to-teal-600 transition-all duration-300 group-hover:w-full" />
                </div>
              </Link>
            ))}
        </div>
