...
const socialIcons: Icons[] = [
  {
    name: 'YouTube',
    icon: (
      <svg role="img" fill="currentColor" className="h-6 w-6 sm:h-8 sm:w-8" viewBox="0 0 24 24">
        <title>YouTube</title>
  ...
  },
  {
    name: 'LinkedIn',
    icon: (
      <svg fill="currentColor" className="h-6 w-6 sm:h-8 sm:w-8" role="img" viewBox="0 0 24 24">
  ...
  return (
    <footer className="border-t">
      <motion.div
        ...
        whileInView="show"
        viewport={{ once: true }}
        className="grid grid-cols-1 gap-12 px-6 py-16 md:grid-cols-4"
      >
        {/* Logo + About */}
        <motion.div variants={fadeUp}>
          <div className="mb-6 flex items-center gap-1">
            ...
            <Link
              href="/"
              className="font-heading text-xl font-semibold tracking-tight lg:text-3xl"
            >
              Repurfy
            </Link>
          </div>
          <p className="text-sm leading-relaxed text-gray-400">
            {/* AI-powered content repurposing to help creators publish everywhere in seconds — without
            burnout. */}
          ...
          <div className="mt-6 flex gap-4">
            {socialIcons.map((social, index) => (
              <div
                key={index}
                className="text-text-secondary flex h-5 w-5 flex-col items-center gap-2"
              >
                {social.icon}
              </div>
            ))}
        ...
        {/* Company */}
        <motion.div variants={fadeUp}>
          <h3 className="mb-4 font-sans font-semibold">Company</h3>
        ...
        {/* Help */}
        <motion.div variants={fadeUp}>
          <h3 className="mb-4 font-sans font-semibold">Help</h3>
          <ul className="space-y-3 text-sm">
            <li>
              <Link href="#" className="hover:">
                Support
        ...
        {/* Newsletter */}
        <motion.div variants={fadeUp}>
          <h3 className="mb-4 font-sans font-semibold">Stay Updated</h3>
          <div className="flex items-center gap-2">
            <Input
              type="email"
              placeholder="hello@repurfy.com"
              className="border-accent placeholder:text-shadow-muted"
            />
            <Button className="rounded-md px-4">→</Button>
          </div>
          <p className="mt-3 text-xs text-gray-500">No spam. Only value.</p>
        </motion.div>
      </motion.div>

      {/* Bottom Text */}
      <div className="border-t border-zinc-400/30 py-6 text-center text-sm text-gray-500 dark:border-white/10">
